/* 07(SDL显示ffmpeg从avi提取的yuv数据(05+06, 05作为一个类))增加暂停和快进快退 */
// 空格-暂停继续；左右键-快退快进3s(随便改了下测试用的，代码没那么规范)

// g++ 07__extract_yuv_2_SDL_reforward.cc -lSDL2 -lavformat -lswscale -lavcodec -lavutil
...
public:
    ...
    double get_video_clock(){
        return this->frame->pts * av_q2d(this->fmt_ctx->streams[this->video_index]->time_base);
    };
    ...
    while(running){
        ...
        while(SDL_PollEvent(&event) || running==3){
            if (event.type == SDL_QUIT){
                ...
                running ^= 2;
            }else if(event.type==SDL_KEYDOWN && event.key.keysym.sym==SDLK_LEFT){   // 增加快退
            ...
            }else if(event.type==SDL_KEYDOWN && event.key.keysym.sym==SDLK_RIGHT){   // 增加快进
                int64_t seek_pos = av_rescale_q((ex_yuv.get_video_clock()+3)*AV_TIME_BASE, 
                    AV_TIME_BASE_Q, ex_yuv.fmt_ctx->streams[ex_yuv.video_index]->time_base);
                av_seek_frame(ex_yuv.fmt_ctx, ex_yuv.video_index, seek_pos, AVSEEK_FLAG_BACKWARD);
                avcodec_flush_buffers(ex_yuv.codec_ctx);
            }
