    ...
    while(1){
        ...
        if (av_read_frame(this->fmt_ctx, pkt) < 0){
            ...
            if (pkt->stream_index == this->v_index){    // 视频流
                ...
                av_log(nullptr, AV_LOG_INFO, "other pkt->pts %ld, a %ld, v %ld\n", pkt->stream_index, this->a_index, this->v_index);
            }
        }
    }
    return 0;
